...
1. Developers → API Keys → Copy publishable key and secret key
2. Connect → Settings → Account types → Enable Express accounts
3. Developers → Webhooks → Add endpoint:
   ...
   - Events to listen for:
     ...
     - `payout.paid`
     - `account.updated`
4. Copy Webhook Signing Secret → go back to Vercel → Settings → Environment Variables → add `STRIPE_WEBHOOK_SECRET` → Redeploy
...
~$0–20/month until meaningful volume
