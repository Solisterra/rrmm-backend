...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (u.bid_status !== "verified")
    return res
      .status(403)
      .json({ error: "Verified buyer account required to bid" });
  if (!u.stripe_customer_id)
    return res
      .status(400)
      .json({
        error: "Payment method required. Please add a card to your account.",
      });

  const { id: auctionId } = req.query as { id: string };
  const { amount, proxyMax } = req.body as {
    amount?: string | number;
    proxyMax?: string | number;
  };
