...
import { supabaseAdmin, getUserFromRequest, formatUser } from "../../../lib/supabase";
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { capability, name, portfolioUrl, focus, followerCount, about, studio, handle } = req.body as {
  ...
  if (current === "pending")
    return res.status(409).json({ error: "This request is already under review." });
  ...
  if (capability === "buyer") {
    ...
    if (!u.stripe_account_id) {
      try {
        const account = await createConnectAccount(u.email, name ?? undefined, u.handle ?? undefined);
  ...
  for (const admin of (admins as Array<{ id: string }> | null) ?? []) {
    await supabaseAdmin.from("notifications").insert({
      user_id: admin.id,
      type: "new_listing",
      title: capability === "buyer" ? "🙋 New bidder verification" : "📷 New seller verification",
