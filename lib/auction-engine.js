...
export async function placeBid({
  ...
  if (auction.status !== "active")
    return { error: `Auction is ${auction.status}` };
  ...
  if (bidderId === auction.photographer_id)
  ...
  if (
    currentWinner?.is_proxy &&
    currentWinner.proxy_max > amount &&
    currentWinner.bidder_id !== bidderId
  ) {
    const counterBid = Math.min(amount + 50, currentWinner.proxy_max);
    await _recordBid({
      auctionId,
      bidderId: currentWinner.bidder_id,
      amount: counterBid,
      isProxy: true,
      proxyMax: currentWinner.proxy_max,
      isWinning: true,
    });
    await _recordBid({
      auctionId,
    ...
    await supabaseAdmin
      .from("auctions")
      .update({ current_bid: counterBid, bid_count: auction.bid_count + 2 })
      .eq("id", auctionId);
...
export async function activateAuction(auctionId) {
  ...
  if (!auction || auction.status !== "pending")
    return { error: "Not a pending auction" };

  const startsAt = new Date();
  const endsAt = new Date(
    startsAt.getTime() + auction.duration_hours * 60 * 60 * 1000,
  );
  ...
  for (const buyer of buyers || []) {
    await supabaseAdmin.from("notifications").insert({
      ...
      body: `"${auction.title}" is now live for bidding.`,
    });
  }
